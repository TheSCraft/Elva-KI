...
constexpr auto anzahleingangsneuronen = 10;
...
constexpr auto anzahltraningsdaten = 6;
...
int main() {
    ...
    double training_inputs[anzahltraningsdaten][anzahleingangsneuronen] = {0};
    ...
    getline(train, in);
    train.close();
    std::strcpy(inputsc, in.c_str());
    for (int i = 0; i < anzahltraningsdaten; i++)
    {
        for (int o = 0; o < anzahleingangsneuronen; o++)
        {
            if ((int)inputsc[tum] ==124 ) {
                tum++;
                if ((int)inputsc[tum] == 98) {
                    training_outputs[i][0] = 1;
                    training_outputs[i][1] = 0;
                }
                if ((int)inputsc[tum] == 118) {
                    training_outputs[i][0] = 0;
                    training_outputs[i][1] = 1;
                }
                tum+=2;
                break;
            }
            cout << inputsc[tum];
            training_inputs[i][o] = ((double)inputsc[tum] - 97)/100*3+0.03;
            cout << training_inputs[i][o] << endl;
            tum++;
        }
    }
